...
from utils.prob_utils import nll_loss, mis_loss, Gaussian2d, to5d

from lstm_train_test import EncoderRNN, DecoderRNN
...
def parse_arguments() -> Any:
    ...
    parser.add_argument(
        ...
        default="nll",
        type=str,
        help="loss function. can be nll or mis",
    )
    parser.add_argument(
        "--normalize",
        action="store_true",
...
def train(
    ...
    for i, (_input, target, helpers) in enumerate(train_loader):
        ...
        decoder_input = to5d(encoder_input)
        ...
        for di in range(rollout_len):
            ...
            # Update loss
            loss += criterion(output[:, :5], target[:, di, :2], scale=scale)
...
def main():
    ...
    if args.model_path is not None and os.path.isfile(args.model_path):
        ...
        print("No model path. starting new model")
        start_epoch = 0
        start_rollout_idx = 0
    ...
    if not args.test:
        ...
        Parallel(n_jobs=-2, verbose=2)(
            delayed(infer_helper)(args, test_data_subsets[i], i, encoder, decoder,
